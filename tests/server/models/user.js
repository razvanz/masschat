...
describe("User Model", function () {
	...
	it('should give a hint of available usernames', function (done) {
		User.schema.statics.findUniqueUsername('test1', '', function (alternative) {
			expect(alternative)
				.not.toEqual(null);
			done();
		});
	...
	it('should throw error if trying to insert a user with an existing username', function (done) {
	...
	it('should throw error if trying to update a nonexisting user', function (done) {
		User.update({
			username: 'not_there'
		}, {}, function (err, theUser) {
			expect(theUser).toEqual(null);
			done();
		});
	});
	...
	it('should throw error if trying to remove a nonexisting user', function (done) {
		User.remove({
			username: 'not_there'
		}, function (err, theUser) {
			expect(err)
