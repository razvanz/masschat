...
describe('GroupChat preconditions', function () {
	...
	describe("GroupChat Model", function () {
		...
		it('should query all the group chats based on criterias', function (done) {
		...
		it('should throw error if trying to insert a group chat without owner', function (done) {
			var anotherChat = {
				users: [],
				messages: [],
		...
		it('should throw error if trying to update a nonexisting group chat', function (done) {
			GroupChat.update({
				chatname: 'not_there'
			}, {}, function (err, chat) {
				expect(err)
		...
		it('should throw error if trying to remove a nonexisting group chat', function (done) {
			GroupChat.remove({
				chatname: 'not_there'
			}, function (err, chat) {
				expect(err)
		...
		describe('Aditional functionality', function () {
			...
			it('should mark all messages before a timestamp as seen', function (done) {
				var messages = [];
				for (var i = 10; i > 0; i--) {
					messages.push({
						author: theUser1._id,
						...
						.getTime(), theUser2._id, function (err, updatedNo) {
							...
							GroupChat.all(function (err, chats) {
								for (var i = 0, j = chats[0].messages.length; i < j; i++) {
									expect(chats[0].messages[i].seenBy).toContain(theUser2._id);
								}
								done();
							});
						});
			...
			it('should retrieve latest n messages from a group chat', function (done) {
				var messages = [],
					lastTmstp, n = 5;
				for (var i = 10; i > 0; i--) {
					lastTmstp = new Date()
			...
			it('should retrieve latest n messages before timestamp from a group chat', function (done) {
				var messages = [],
